...
pub(crate) const BIN_NAME: &str = "fj";

#[derive(Parser, Debug)]
#[command(name = "fj", version)]
struct App {
    /// Directory containing fj configuration and credentials
    #[clap(long, global = true, env = "FORGEJO_CONFIG", hide_env_values = true)]
...
pub fn cli_command() -> clap::Command {
    <App as clap::CommandFactory>::command()
}
...
impl Command {
    async fn run(self, keys: &mut crate::KeyInfo, host_name: Option<&str>) -> eyre::Result<()> {
        match self {
...
fn get_editor_and_flags() -> eyre::Result<(PathBuf, Vec<String>)> {
    ...
    let source = if from_git { "core.editor" } else { "$EDITOR" };
    let mut args =
        shlex::split(&editor_str).ok_or_else(|| eyre!("{source} contains erroneous command"))?;

    if args.is_empty() {
        eyre::bail!("{source} is empty");
    }
...
pub(crate) fn render_label(label: &forgejo_api::structs::Label) -> eyre::Result<String> {
    use std::fmt::Write;
...
pub(crate) fn render_label_list(labels: &[forgejo_api::structs::Label]) -> eyre::Result<()> {
    if labels.is_empty() {
...
pub(crate) async fn edit_labels(
